...
void olsrTxCallback(dwDevice_t *dev) {
  ...
  DEBUG_PRINT_OLSR_SEND("Departure timestamp for seq %u is %llu\n",
                        txOlsrPkt->m_packetHeader.m_packetSeq,
                        departure.full);
}
    ...
    if (helloMsgBody->m_linkMessage[i].m_addresses == myAddress) {
      if (lt == OLSR_LOST_LINK) {
        ...
        olsrLinkSet.setData[linkTuple].data.m_expirationTime =
            olsrLinkSet.setData[linkTuple].data.m_symTime + OLSR_NEIGHB_HOLD_TIME;
        updated = true;
      } else {
...
void populateNeighborSet(const olsrMessage_t *helloMsg) {
  setIndex_t nbTuple = olsrFindNeighborByAddr(&olsrNeighborSet,
                                              helloMsg->m_messageHeader.m_originatorAddress);
  if (nbTuple != -1) {
    olsrNeighborSet.setData[nbTuple].data.m_willingness =
        ((olsrHelloMessageHeader_t *) helloMsg->m_messagePayload)->m_willingness;
    ...
    if (olsrIndex->m_messageHeader.m_messageType == HELLO_MESSAGE) {
      ...
      for (int i = 0; i < length; i++) {
        DEBUG_PRINT_OLSR_HELLO("%s: link %d :addr:%d ,link code is %d\n",
                               msgStr,
                               i,
                               helloMessage->m_linkMessage[i].m_addresses,
                               helloMessage->m_linkMessage[i].m_linkCode);
      }
      ...
      for (int i = 0; i < count; i++) {
        DEBUG_PRINT_OLSR_TC("%s: %d: address %d .distance is %d",
                            msgStr,
                            i,
                            tcMessage->m_content[i].m_address,
                            tcMessage->m_content[i].m_distance);
      }
      ...
      DEBUG_PRINT_OLSR_TS("%s: TS from %u (seq:%u) len %u, speed %d\n",
                          msgStr,
                          tsMessageHeader->m_originatorAddress,
                          tsMessageHeader->m_messageSeq,
                          tsMessageHeader->m_messageSize,
                          tsMessageHeader->m_velocity);
      ...
      for (olsrTsMessageBodyUnit_t *tsMsgBodyUnit = (olsrTsMessageBodyUnit_t *) msgPtr; tsMsgBodyUnit < msgPtrEnd;
        ...
        DEBUG_PRINT_OLSR_TS("%s: Peer %u (seq:%u), timestamp: %llu\n",
                            msgStr,
                            tsMsgBodyUnit->m_tsAddr,
                            tsMsgBodyUnit->m_sequence,
                            timestamp.full);
      }
    }
  }
...
void olsrRoutingTableComputation2() {
  ...
  while (neighborIt != -1) {
    if (olsrNeighborSet.setData[neighborIt].data.m_status == STATUS_SYM) {
      ...
      while (linkIt != -1) {
        if (olsrNeighborSet.setData[neighborIt].data.m_neighborAddr
            == olsrLinkSet.setData[linkIt].data.m_neighborAddr) {
  ...
  while (neighbor2It != -1) {
    ...
    while (neighborIt != -1) {
      if (olsrNeighborSet.setData[neighborIt].data.m_neighborAddr
          == olsrTwoHopNeighborSet.setData[neighbor2It].data.m_twoHopNeighborAddr) {
        is_n1 = true;
        break;
      }
      if (olsrNeighborSet.setData[neighborIt].data.m_neighborAddr
          == olsrTwoHopNeighborSet.setData[neighbor2It].data.m_neighborAddr
    ...
    while (routingIt != -1) {
      ...
      if (tmpRoutingSet.setData[routingIt].data.m_destAddr
          == olsrTwoHopNeighborSet.setData[neighbor2It].data.m_twoHopNeighborAddr) {
        route = NULL;
        break;
      }
...
void olsrPacketDispatch(const packetWithTimestamp_t *rxPacketWts) {
  ...
  while (index < lengthOfPacket) {
    ...
    if (duplicated == -1) {
      switch (type) {
        case HELLO_MESSAGE:DEBUG_PRINT_OLSR_HELLO("recv a hello\n");
          olsrProcessHello((olsrMessage_t *) message);
          break;
        case TC_MESSAGE:DEBUG_PRINT_OLSR_RECEIVE("recv a TC\n");
          olsrProcessTc((olsrMessage_t *) message);
          break;
        case DATA_MESSAGE:DEBUG_PRINT_OLSR_RECEIVE("DATA_MESSAGE\n");
          olsrProcessData((olsrMessage_t *) message);
          break;
        case TS_MESSAGE:DEBUG_PRINT_OLSR_RECEIVE("TS_MESSAGE\n");
          g_ts_receive_count++;
          olsrProcessTs((olsrMessage_t *) message, rx_otimestamp);
          break;
        default:DEBUG_PRINT_OLSR_RECEIVE("WRONG MESSAGE\n");
          break;
      }
    } else {
      doForward = false;
