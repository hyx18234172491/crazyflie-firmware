...
void helloTrans(Ranging_Message_t *ranging)
{
  helloMessage.m_messageHeader.m_messageType = 1;
  helloMessage.m_messageHeader.m_vTime = 0xffffff;
  helloMessage.m_messageHeader.m_messageSize = sizeof(olsrMessageHeader_t);
  helloMessage.m_messageHeader.m_originatorAddress = ranging->header.srcAddress;
  helloMessage.m_messageHeader.m_destinationAddress = myAddress;
  helloMessage.m_messageHeader.m_relayAddress = myAddress;
  helloMessage.m_messageHeader.m_timeToLive = 0xff;
  helloMessage.m_messageHeader.m_hopCount = 0;
  helloMessage.m_messageHeader.m_messageSeq = getMessageSeqNumber();

  olsrHelloMessage_t temp;
  temp.m_helloHeader.m_hTime = OLSR_HELLO_INTERVAL; // hello's header on packet
  temp.m_helloHeader.m_willingness = WILL_ALWAYS;
  temp.m_helloHeader.m_linkMessageNumber = 0;
  uint8_t bodyUnitCount = (ranging->header.msgLength - sizeof(Ranging_Message_Header_t)) / sizeof(Body_Unit_t);
  olsrLinkMessage_t linkMessage;
  for(int i = 0;i<bodyUnitCount;i++){
   //linkMessage.m_linkCode = (linkType & 0x03) | ((nbType << 2) & 0x0f);
   linkMessage.m_addressUsedSize = 1;
   linkMessage.m_addresses = ranging->bodyUnits->address;
   linkMessage.m_weight = distanceToWeight(ranging->bodyUnits->distance);
   temp.m_linkMessage[temp.m_helloHeader.m_linkMessageNumber++] = linkMessage;
  }

  uint16_t writeSize = sizeof(olsrHelloMessageHeader_t) + temp.m_helloHeader.m_linkMessageNumber *                                                          sizeof(olsrLinkMessage_t);
  helloMessage.m_messageHeader.m_messageSize += writeSize;
  memcpy(helloMessage.m_messagePayload, &helloMessage, writeSize);
}
...
void processRangingMessage(Ranging_Message_With_Timestamp_t *rangingMessageWithTimestamp)
{
  ...
  if (xTaskGetTickCount()-lastHelloTime > M2T(HELLO_INTERVAL))
  {
    lastHelloTime = xTaskGetTickCount();
    helloTrans(rangingMessage);
    processHelloMessage(&helloMessage);
  }
  else
  {
    /* handle new neighbor */
